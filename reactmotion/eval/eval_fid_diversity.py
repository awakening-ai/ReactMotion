...
def collect_real_embeddings(
    ...
    """
    从 new_joint_vecs 中提取真实 motion embedding (EvaluatorModelWrapper 空间)。
    split='all' 时合并 train/val/test 三个 CSV，获得更多样本以稳定 FID 协方差估计。
...
def collect_gen_embeddings(
    ...
    """
    收集生成 motion 的 EvaluatorModelWrapper embedding。
    支持两种格式:
      eval_dump  — run_eval_gen_dump.sh 输出 (group=*/*.motion_codes.npy)
      qwen_t2m   — qwen_t2m_pipeline.py 输出 (tokens/<key>/m*.npy)
    VQ-VAE decoder 输出已处于归一化空间, 无需额外归一化。
    """
...
def main():
    ...
    ap.add_argument(
        ...
        type=str,
        required=True,
        help="生成结果根目录. eval_dump: 包含 group=* 子目录; qwen_t2m: 包含 tokens/<key>/m*.npy",
    )
    ap.add_argument(
        "--gen_format",
        type=str,
        ...
        help="eval_dump: run_eval_gen_dump.sh 格式; qwen_t2m: qwen_t2m_pipeline.py 格式",
    )
    ap.add_argument(
        "--dataset_dir",
        type=str,
    ...
    ap.add_argument(
        ...
        help="用哪个 split 做 real 参考分布. 'all' 合并 train+val+test (样本更多, FID 更稳定). "
             "需要 --pairs_csv 为目录.",
    )
    ap.add_argument(
        "--t2m_opt",
